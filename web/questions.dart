class Questions {
  final List<Map> questions = [
    {
      'question':
          'Rasululloh alayhissolatu wassalam milodiy qaysi yilda tavallud topganlar?',
      'answers': [
        '571',
        '583',
        '566',
        '573',
      ],
      'correctAnswer': '571',
    },
    {
      'question': 'Islom arkonlari nimalardan iborat?',
      'answers': [
        'Shahodat kalimasi, Namoz, Hajj, Ota-onani hurmat qilish',
        "Namoz, Ro'za, Hajj, Zakot, Sadaqa",
        "Shahodat kalimasi, Namoz, Ro'za, Zakot, Hajj",
        'Islom, Iymon, Ehson, Namoz',
      ],
      'correctAnswer': "Shahodat kalimasi, Namoz, Ro'za, Zakot, Hajj",
    },
    {
      'question': 'Iymonni ustunlari qaysilar?',
      'answers': [
        "Allohga, Farishtalarga, Ilohiy kitoblarga, Payg'ambarlarga iymon",
        "Allohga, Payg'ambarlarga, Oxirat kuniga iymon",
        "Allohga, Farishtalarga, Ilohiy kitoblarga, Payg'ambarlarga, Oxirat kuniga, Qazoi qadarga iymon",
        "Allohga, Oxiratga, Payg'ambarlarga, Farishtalarga iymon",
      ],
      'correctAnswer':
          "Allohga, Farishtalarga, Ilohiy kitoblarga, Payg'ambarlarga, Oxirat kuniga, Qazoi qadarga iymon",
    },
    {
      'question': "Quron Karimning birinchi oyatlari qaysi oyda nozil bo'lgan?",
      'answers': [
        'Muharram',
        'Safar',
        'Ramazon',
        'Shawwal',
      ],
      'correctAnswer': 'Ramazon',
    },
    {
      'question':
          "Rasululloh alayhissolatu wassalam qaysi g'orda birinchi oyatlarni qabul qilib olganlar?",
      'answers': [
        'Jabal',
        'Hiro',
        'Kara',
        'Nuur',
      ],
      'correctAnswer': 'Hiro',
    },
    {
      'question': 'Quron Karimdagi qaysi suradan oldin basmala aytilmaydi?',
      'answers': [
        'Tavba',
        'Mulk',
        'Fotiha',
        'Moida',
      ],
      'correctAnswer': 'Tavba',
    },
    {
      'question': 'Islomda amallar qanday hukmlarga egadurlar?',
      'answers': [
        'Farz, Sunna, Harom',
        'Farz, Vojib, Sunna, Harom',
        'Farz, Vojib, Sunna, Mustahab, Makruh, Harom, Muboh',
        'Farz, Sunna, Makruh, Harom',
      ],
      'correctAnswer': 'Farz, Vojib, Sunna, Mustahab, Makruh, Harom, Muboh',
    },
    {
      'question': "Farz al-Ayn qanday ma'noga ega?",
      'answers': [
        'Har bir musulmon uchun bajarilishi shart amallar',
        'Bir necha musulmonlar ado etsa, qolganlarning zimmasidan soqit amallar',
        'Faqat erkak kishilar uchun farz amallar',
        "Birorta javob to'g'ri emas",
      ],
      'correctAnswer': 'Har bir musulmon uchun bajarilishi shart amallar',
    },
    {
      'question': 'Tahoratning farzlari qaysilar?',
      'answers': [
        "Niyat, Yuzni yuvish, Qo'lni tirsakkacha yuvish, Oyoqni yuvish",
        "Yuzni yuvish, Qo'lni tirsak bilan qo'shib yuvmoqlik, Boshga mahs tortish, Oyoqni boldir suyagigacha yuvish",
        "Bismillohni aytish, Yuz yuvmoqlik, Qo'llni tirsakkacha yuvish, Oyoqqa mahs tortish",
        "Qo'l kaftini yuvmoqlik, Yuzni yuvish, Boshga mahs tortish",
      ],
      'correctAnswer':
          "Yuzni yuvish, Qo'lni tirsak bilan qo'shib yuvmoqlik, Boshga mahs tortish, Oyoqni boldir suyagigacha yuvish",
    },
    {
      'question':
          "Imom Shofei rahimahulloh qaysi kitob haqida: 'Yer yuzida Allohning kitobidan keyingi eng sahih kitob' deganlar?",
      'answers': [
        'Sahih Buxoriy',
        'Sahih Muslim',
        'Musnad Shofei',
        'Muwatta',
      ],
      'correctAnswer': 'Muwatta',
    },
    {
      'question': "Qur'oni Karim nechta suradan iborat?",
      'answers': [
        '113',
        '115',
        '112',
        '114',
      ],
      'correctAnswer': '114',
    },
    {
      'question': "Qur'oni Karimda nechta payg'ambarlar haqida habar kelgan?",
      'answers': [
        '18',
        '25',
        '42',
        '33',
      ],
      'correctAnswer': '25',
    },
    {
      'question': "Qur'oni Karimdagi eng uzun sura qaysi?",
      'answers': [
        'Baqara',
        'Moida',
        'Anbiya',
        'Tawba',
      ],
      'correctAnswer': 'Baqara',
    },
    {
      'question': "Qur'oni Karimdagi eng qisa sura qaysi?",
      'answers': [
        'Ixlos',
        'Kavsar',
        'Falaq',
        'Nas',
      ],
      'correctAnswer': 'Kavsar',
    },
    {
      'question':
          "Qur'oni Karimning qaysi sura oyatlari birinchi bo'lib nozil bo'lgan?",
      'answers': [
        'Ixlos',
        'Iqro',
        'Fotiha',
        'Kavsar',
      ],
      'correctAnswer': 'Iqro',
    },
    {
      'question': "Qur'oni Karim necha yil davomida nozil bo'lgan?",
      'answers': [
        '30',
        '18',
        '23',
        '45',
      ],
      'correctAnswer': '23',
    },
    {
      'question': "Qur'oni Karimdagi qaysi sura Ari (asal ari) deb nomlangan?",
      'answers': [
        'Al-Naml',
        'Al-Qoriah',
        'Al-Nahl',
        'Toha',
      ],
      'correctAnswer': 'Al-Nahl',
    },
    {
      'question':
          "Qur'oni Karim qaysi halifa davrida kitob shakliga keltirilgan?",
      'answers': [
        'Umar rodhiyallohu anhu',
        'Ali rodhiyallohu anhu',
        'Abu Bakr rodhiyallohu anhu',
        'Usmon rodhiyallohu anhu',
      ],
      'correctAnswer': 'Abu Bakr rodhiyallohu anhu',
    },
    {
      'question': 'Nabiylardan qaysi biri duradgorlik kasbi egasi edilar?',
      'answers': [
        'Zakariyo alayhissalam',
        'Dowud alayhissalam',
        'Muso alayhissalam',
        'Idris alayhissalam',
      ],
      'correctAnswer': 'Zakariyo alayhissalam',
    },
    {
      'question': "Birinchi muazzin (azon aytuvchi) kim bo'lgan?",
      'answers': [
        'Bilol rodhiyallohu anhu',
        'Zayd rodhiyallohu anhu',
        'Ali rodhiyallohu anhu',
        'Umar rodhiyallohu anhu',
      ],
      'correctAnswer': 'Bilol rodhiyallohu anhu',
    },
    {
      'question':
          'Bilol rodhiyallohu anhu hazratlarini kim qullikdan ozod qilgan?',
      'answers': [
        'Abu Hurayra rodhiyallohu anhu',
        'Abu Bakr rodhiyallohu anhu',
        'Ali rodhiyallohu anhu',
        'Ammar rodhiyallohu anhu',
      ],
      'correctAnswer': 'Abu Bakr rodhiyallohu anhu',
    },
    {
      'question': 'Madina shahri avval qanday nom bilan atalgan?',
      'answers': [
        'Misr',
        'Yasrib',
        'Suriya',
        'Sharq',
      ],
      'correctAnswer': 'Yasrib',
    },
    {
      'question':
          'Rasululloh alayhissolatu wassalamning tuyalarining ismi nima edi?',
      'answers': [
        'Baqsa',
        'Urwa',
        'Qaswa',
        'Kalba',
      ],
      'correctAnswer': 'Qaswa',
    },
    {
      'question':
          'Rasululloh alayhissolatu wassalamning otalarining ismi nima?',
      'answers': [
        'Qasim',
        'Ibrohim',
        'Hashim',
        'Abdulloh',
      ],
      'correctAnswer': 'Abdulloh',
    },
    {
      'question':
          'Rasululloh alayhissolatu wassalamning onalarining ismi nima?',
      'answers': [
        'Amina',
        'Zaynab',
        'Fatima',
        'Ruqayyah',
      ],
      'correctAnswer': 'Amina',
    },
    {
      'question':
          'Rasululloh alayhissolatu wassalamning birinchi ayollarining ismi nima?',
      'answers': [
        'Aisha',
        'Rayhana',
        'Hadicha',
        'Sawda',
      ],
      'correctAnswer': 'Hadicha',
    },
    {
      'question':
          "Rasululloh alayhissolatu wassalam necha yoshda payg'ambar bo'ldilar?",
      'answers': [
        '41',
        '38',
        '45',
        '40',
      ],
      'correctAnswer': '40',
    },
    {
      'question':
          'Rasululloh alayhissolatu wassalam onalari vafot etganda necha yoshda edilar?',
      'answers': [
        '8',
        '6',
        '9',
        '12',
      ],
      'correctAnswer': '6',
    },
    {
      'question': "Qaysi payg'ambar 'Kalimulloh' maqomiga ega?",
      'answers': [
        'Ibrohim ayhissalom',
        'Dovud alayhissalom',
        'Muso alayhissalom',
        'Iso alayhissalom'
      ],
      'correctAnswer': 'Muso alayhissalom',
    },
    {
      'question':
          'Rasululloh alayhissolatu wassalamni Madinada kutib olib islomga kirgan insonlar qanday ataladi?',
      'answers': [
        'Ansor',
        'Muhojir',
        'Tobein',
        'Sahoba',
      ],
      'correctAnswer': 'Ansor',
    },
    {
      'question': 'Hijratning 2-yilida yuz bergan jangning nomi nima?',
      'answers': [
        'Uhud',
        'Badr',
        'Tabuk',
        'Xandaq',
      ],
      'correctAnswer': 'Badr',
    },
    {
      'question': 'Islomni birinchi qabul qilgan bola kim?',
      'answers': [
        'Ali rodhiyallohu anhu',
        'Abu Bakr rodhiyallohu anhu',
        'Ibn Umar rodhiyallohu anhu',
        'Usmon rodhiyallohu anhu',
      ],
      'correctAnswer': 'Ali rodhiyallohu anhu',
    },
    {
      'question': 'Islomni birinchi qabul qilgan kishi kim?',
      'answers': [
        'Ali rodhiyallohu anhu',
        'Abu Bakr rodhiyallohu anhu',
        'Ibn Umar rodhiyallohu anhu',
        'Usmon rodhiyallohu anhu',
      ],
      'correctAnswer': 'Abu Bakr rodhiyallohu anhu',
    },
    {
      'question': 'Musulmonlar uchun birinchi Qibla qaysi edi?',
      'answers': [
        'Kaaba',
        'Masjid al-Aqso',
        'Masjid an-Nabavi',
        "Tog'ri javob yo'q",
      ],
      'correctAnswer': 'Masjid al-Aqso',
    },
    {
      'question': 'Quron Karimdagi ayol ismi bilan atalgan sura qaysi?',
      'answers': [
        'Niso',
        'Anbiyo',
        'Maryam',
        'Zuho',
      ],
      'correctAnswer': 'Maryam',
    },
    {
      'question': 'Muboh qanday amallarni anglatadi?',
      'answers': [
        "Qilsa savob ham, qilmasa gunoh ham yo'q amal",
        "Qilsa savob, qilmasa gunoh yo'q amal",
        'Qilsa gunoh, qilmasa savob amal',
        "To'g'ri javob yo'q",
      ],
      'correctAnswer': "Qilsa savob ham, qilmasa gunoh ham yo'q amal",
    },
    {
      'question': 'Istinjo islom dinida nimani anglatadi?',
      'answers': [
        "Qo'lni tirsakkacha yuvish",
        'Avratlarni (orqa va oldi) yuvish',
        'Boshga mahs tortish',
        "Oyoqni to'piqqacha yuvish",
      ],
      'correctAnswer': 'Avratlarni yuvish',
    },
    {
      'question':
          "Tahorat uchun niyat va tahorat paytida og'izni uch marta chayishning hukmi nima?",
      'answers': [
        'Sunna va Sunna',
        'Vojib va Sunna',
        'Farz va Vojib',
        'Muboh va Sunna',
      ],
      'correctAnswer': 'Sunna, Sunna',
    },
    {
      'question': 'Quyidagilardan qaysi biri tahoratni buzmaydi?',
      'answers': [
        'Tanadan qon oqishi',
        "Mast bo'lish",
        "Jinsiy a'zoni ushlash",
        'Hushdan ketmoq',
      ],
      'correctAnswer': "Jinsiy a'zoni ushlash",
    },
    {
      'question': "G'uslning farzlari nimalardan iborat?",
      'answers': [
        'Og‘izni yuvmoq, Burunni achishtirib chaymoq, Butun tanani ishqalab yuvmoq',
        'Niyat qilish, Butun tanani yuvish',
        'Bismillohni aytish, Niyat qilish, Yuzni yuvmoq, Butun tanani yuvish',
        "Yaxshilab cho'milsa bo'ldi",
      ],
      'correctAnswer':
          'Og‘izni yuvmoq, Burunni achishtirib chaymoq, Butun tanani ishqalab yuvmoq',
    },
    {
      'question':
          "Rasululloh alayhissolatu wassalamni onalari vafotidan keyin kimning qaramog'ida qoldilar?",
      'answers': [
        'Otalari Abdulloh',
        'Bobolari AbdulMutallib',
        'Amakilar Abu Tolib',
        "Bir o'zlari yashadilar",
      ],
      'correctAnswer': 'Bobolari AbdulMutallib',
    },
    {
      'question':
          "Rasululloh alayhissolatu wassalamning nechta qizlari bo'lgan?",
      'answers': [
        '3',
        '5',
        '2',
        '4',
      ],
      'correctAnswer': '4',
    },
    {
      'question':
          "Rasululloh alayhissolatu wassalamning nechta o'g'illari bo'lgan?",
      'answers': [
        '3',
        '5',
        '2',
        '4',
      ],
      'correctAnswer': '3',
    },
    {
      'question':
          'Dovud alayhissalomga berilgan ilohiy kitob qandan nomlanadi?',
      'answers': [
        'Injil',
        'Zobur',
        'Taurot',
        'Veda',
      ],
      'correctAnswer': 'Zobur',
    },
    {
      'question': 'Muso alayhissalomga berilgan ilohiy kitob qandan nomlanadi?',
      'answers': [
        'Injil',
        'Zobur',
        'Taurot',
        'Veda',
      ],
      'correctAnswer': 'Taurot',
    },
    {
      'question': 'Iso alayhissalomga berilgan ilohiy kitob qandan nomlanadi?',
      'answers': [
        'Injil',
        'Zobur',
        'Taurot',
        'Veda',
      ],
      'correctAnswer': 'Injil',
    },
    {
      'question': """Alloh subhanahu wa ta'alaning qaysi buyuk ismi 
          'Kofirga ham mo'minga ham mehribon, ne'mat beruvchi' ma'nosini anglatadi?""",
      'answers': [
        'ar-Rahim',
        'ar-Rohman',
        'al-Quddus',
        'al-Aziz',
      ],
      'correctAnswer': 'ar-Rohman',
    },
    {
      'question': "Qaysi payg'ambarni baliq yutib yuborgan?",
      'answers': [
        'Yusuf alayhissalom',
        'Muso alayhissalom',
        'Nuh alayhhissalom',
        'Yunus alayhissalom',
      ],
      'correctAnswer': 'Yunus alayhissalom',
    },
    {
      'question':
          "'Allohumma inniy a’uzu bika minal xubsi val xobais' duosi qachon o'qiladi?",
      'answers': [
        'Tahorat olishdan oldin',
        'Uyqudan turganda',
        'Hotajxonaga kirishdan avval',
        'Uxlashdan olidn',
      ],
      'correctAnswer': 'Hotajxonaga kirishdan avval',
    },
    {
      'question':
          'Hojat vaqtida oldi yoki orqasini Qiblaga yuzlantirishning hukmi nima?',
      'answers': [
        'Harom',
        'Makruhi tahrimiy',
        'Makruh tanzihiy',
        'Muboh',
      ],
      'correctAnswer': 'Makruhi tahrimiy',
    },
    {
      'question': "Tahoratsiz kishiga qaysi namozni o'qishga ruxsat bor?",
      'answers': [
        'Nafl nomozlari',
        'Taroweh nomozi',
        "Har qanday namoz o'qish harom",
        'Qazo nomozi',
      ],
      'correctAnswer': "Har qanday namoz o'qish harom",
    },
    {
      'question': "O'qilmasdan qolib ketgan farz namozlarining hukmi qanday?",
      'answers': [
        "Qazo qilib o'qish sunnat",
        "Qazo qilib o'qish farz",
        "Qazo qilib o'qish makruh",
        "To'gri javob yo'q",
      ],
      'correctAnswer': "Qazo qilib o'qish farz",
    },
    {
      'question': 'Musofir va muqim insonga mahs tortish muddati qancha?',
      'answers': [
        '3 kecha-kunduz va 1 kecha-kunduz',
        'Ikki holatta ham 1 kun',
        'Ikki holatta ham 3 kun',
        "Inson o'zi hohlaganicha",
      ],
      'correctAnswer': '3 kecha-kunduz va 1 kecha-kunduz',
    },
    {
      'question': 'Mahsiga mahs tortish shartlari qanday?',
      'answers': [
        "Mahsi oyoqni to'piqqacha yopishi, Suv o'tkazmaydigan darajada qalin bo'lishi",
        "Uch barmoq miqdorida yirtiq bo'lmasligi, Yurganda yechilib ketmasligi",
        "Taqriba 6-8 km (1 farsah) yurganda yirtilmas darajada qalin bo'lishi, Mahsining tahorat bilan kiyilishi",
        "Yuqoridagi barcha javoblar to'g'ri",
      ],
      'correctAnswer': "Yuqoridagi barcha javoblar to'g'ri",
    },
    {
      'question': "Mahs tortishning muddati qachondan e'tiborga olinadi?",
      'answers': [
        'Mahsi kiyilgandan boshlab',
        'Tahorat singandan boshlab',
        'Yangi kun boshlanganda',
        "To'g'ri javob yo'q",
      ],
      'correctAnswer': 'Tahorat singandan boshlab',
    },
    {
      'question': 'Tayammum qilishda niyatning hukmi nima?',
      'answers': [
        'Sunna',
        'Farz',
        'Mustahab',
        'Muboh',
      ],
      'correctAnswer': 'Farz',
    },
    {
      'question': "Tayammum qilgan kishi suvni ko'rsa nima qiladi?",
      'answers': [
        'Tayammum buzuladi, tahorat qilishi shart',
        "Tayammum bilan namoz o'qilaveradi",
        "Tahorat qilsa yaxshi, qilmasa gunoh yo'q",
        "To'g'ri javob yo'q",
      ],
      'correctAnswer': 'Tayammum buzuladi, tahorat qilishi shart',
    },
    {
      'question': "Kishiga namoz o'qish farz bo'lishining shartlari qaysilar?",
      'answers': [
        'Tahorat olish, namoz vaqtining kirishi',
        "Musulmonlik, balog'atga yetish, aql",
        "Musulmonlik, balog'atga yetish, tahorat olish",
        'Musulmonlik, tahorat olish, namoz vaqtining kirishi',
      ],
      'correctAnswer': "Musulmonlik, balog'atga yetish, aql",
    },
    {
      'question': "Namozning farz bo'lish sharti nima?",
      'answers': [
        'Namoz vaqtining kirishi',
        'Tahorat olish',
        "Musulmon bo'lish",
        "Balog'at yoshiga yetish",
      ],
      'correctAnswer': 'Namoz vaqtining kirishi',
    },
    {
      'question': 'Shom namozining vaqti qachon?',
      'answers': [
        'Kechqurun soat 5da',
        "Quyosh botishidan qizil shafaq yo'q bo'lgunicha",
        'Masjidda azon etganda',
        "Qizil shafat yo'q bo'lgandan to quyosh chiqqunicha",
      ],
      'correctAnswer': "Quyosh botishidan qizil shafaq yo'q bo'lgunicha",
    },
    {
      'question': "Asr namozining vaqti Abu Hanifa rahimahullohga ko'ra qaysi?",
      'answers': [
        "Narsaning soyasi o'zidan ikki baravar uzun bo'lganidan to quyosh botguncha",
        "Narsani soyasi o'zidan bir baravar uzun bo'lganidan to quyosh botguncha",
        'Quyosh botishidan 40 daqiqa oldin boshlanib to quyosh botguncha',
        "Peshin namozini o'qib bo'lishi bilan boshlanib to quyosh botguncha",
      ],
      'correctAnswer':
          "Narsaning soyasi o'zidan ikki baravar uzun bo'lganidan to quyosh botguncha",
    },
    {
      'question':
          "Asr namozining vaqti Abu Yusuf, Imom Muhammad rahimahullohlarga ko'ra qaysi?",
      'answers': [
        "Narsaning soyasi o'zidan ikki baravar uzun bo'lganidan to quyosh botguncha",
        "Narsani soyasi o'zidan bir baravar uzun bo'lganidan to quyosh botguncha",
        'Quyosh botishidan 40 daqiqa oldin boshlanib to quyosh botguncha',
        "Peshin namozini o'qib bo'lishi bilan boshlanib to quyosh botguncha",
      ],
      'correctAnswer':
          "Narsani soyasi o'zidan bir baravar uzun bo'lganidan to quyosh botguncha",
    },
    {
      'question': 'Vitr namozining hukmi qanday?',
      'answers': [
        'Farz',
        'Vojib',
        'Sunna',
        'Mustahab',
      ],
      'correctAnswer': 'Vojib',
    },
    {
      'question':
          "Qaysi vaqtlarda farz, janoza namozlarini o'qish, tilovat sajdasini qilish joiz emas?",
      'answers': [
        "Quyosh chiqayotgan vaqtdan to ko'ratilguncha, quyosh botayotganda",
        "Quyosh chiqayotgan vaqtdan to ko'ratilguncha, quyosh tikkaga kelganda",
        'Quyosh tikkaga kelganda va quyosh botayotganda',
        'Yuqoridagilarning hammasi',
      ],
      'correctAnswer': 'Yuqoridagilarning hammasi',
    },
    {
      'question':
          "Bomdod va asr namozlarining farzidan keyin nafl namoz o'qishning hukmi nima?",
      'answers': [
        'Mustahab',
        'Vojib',
        'Harom',
        'Makruh',
      ],
      'correctAnswer': 'Makruh',
    },
    {
      'question': 'Azon va iqomaning hukmi nima?',
      'answers': [
        'Erkak va ayollarga namoz oldidan sunna',
        'Erkaklarga sunna, ayollarga makruh',
        'Erkaklarga sunna, ayollarga Vojib',
        'Erkak va ayollarga namoz oldidan muboh',
      ],
      'correctAnswer': 'Erkaklarga sunna, ayollarga makruh',
    },
    {
      'question':
          "Azon eshituvchi uchun 'Hayya alas-Solah, Hayya alal-Falah' kalimalaridan keyin nima afzal?",
      'answers': [
        'Subhanalloh deyishi',
        "O'sha kalimalarni takrorlashi",
        'Alhamdulillah deb aytmoqlik',
        'La havla vala quvvata illa billah deyishi',
      ],
      'correctAnswer': 'La havla vala quvvata illa billah deyishi',
    },
    {
      'question':
          'Quyidagilardan qaysilar namozda mahfiy (ovossiz) holda aytiladi?',
      'answers': [
        'Fotihadan oldingi Basmala, Fotihadan keyingi Amiin, Attahiyat, Salovat va duolar',
        "Ruku va sajdadagi 'Subhana Robbiy al-Azim, Subhana Robbiy al-A'la' kalimalari",
        'Sano duosi, taavvuz (Auzu billahi min ash-shatonir rojim)',
        'Yuqoridagilarning barchasi',
      ],
      'correctAnswer': 'Yuqoridagilarning barchasi',
    },
    {
      'question': "Mihna davri deb qanday davr ta'rif etiladi?",
      'answers': [
        'Mutazila aqidasining rivojlanish va kengayish davri',
        "Mutazila aqidasiga qarshi chiqqanlarni ta'qib qilish, jazolash davri",
        'Ashari aqidasiga asos solingan davr',
        "Birorta javob to'g'ri emas",
      ],
      'correctAnswer':
          "Mutazila aqidasiga qarshi chiqqanlarni ta'qib qilish, jazolash davri",
    },
    {
      'question': 'Ahli sunna va jamoa aqidaviy maktab asoschilar kimlar?',
      'answers': [
        'Imom Abu Hanifa rahimahulloh, Imom Molik rahimahulloh',
        'Imom Ashari rahimahulloh, Imom Moturudi rahimahulloh, Imom Buxori rahimahulloh',
        'Imom Ashari rahimahulloh, Imom Moturudi rahimahulloh, Imom Ahmad rahimahulloh',
        'Imom Ashari rahimahulloh, Imom Moturudi rahimahulloh',
      ],
      'correctAnswer':
          'Imom Ashari rahimahulloh, Imom Moturudi rahimahulloh, Imom Ahmad rahimahulloh',
    },
    {
      'question': 'Mihna davrida qaysi mashhur ulamo qamoqqa tashlangan?',
      'answers': [
        'Imom Muhammad rahimahulloh',
        "Imom Shofe'i rahimahulloh",
        'Imom Ahmad ibn Hanbal rahimahulloh',
        'Imom Ashari rahimahulloh',
      ],
      'correctAnswer': 'Imom Ahmad ibn Hanbal rahimahulloh',
    },
    {
      'question':
          "Ahli sunna va jamoa aqidasini o'z ichiga jamlab olgan eng mashhur kitoblardan biri qaysi?",
      'answers': [
        'Aqida al-Tahawiyya',
        'Aqida as-Sanusiyya',
        'Kitab al-Risola',
        'Aqida al-Wasitiyya',
      ],
      'correctAnswer': 'Aqida al-Tahawiyya',
    },
    {
      'question': "Bismillahir-Rohmanir-Rohim kalimasining ma'nosi nima?",
      'answers': [
        'Mehribon va rahmli Alloh nomi bilan (boshlayman)',
        'Hamd olamlar Robbi - Allohgadur',
        'Jazo mukofot kuni egasidur',
        'U Rohman (Mehribon) va Rohim (Rahmli)',
      ],
      'correctAnswer': 'Mehribon va rahmli Alloh nomi bilan (boshlayman)',
    },
    {
      'question': "Alhamdu lillahi Robbil-'alamin oyatining ma'nosi nima?",
      'answers': [
        'U Rohman (Mehribon) va Rohim (Rahmli)',
        'Mehribon va rahmli Alloh nomi bilan (boshlayman)',
        'Hamd olamlar Robbi - Allohgadur',
        'Jazo mukofot kuni egasidur',
      ],
      'correctAnswer': 'Hamd olamlar Robbi - Allohgadur',
    },
    {
      'question': "Ar-Rohmanir-Rohim oyatining ma'nosi nima?",
      'answers': [
        'U Rohman (Mehribon) va Rohim (Rahmli)',
        'Mehribon va rahmli Alloh nomi bilan (boshlayman)',
        'Hamd olamlar Robbi - Allohgadur',
        'Jazo mukofot kuni egasidur',
      ],
      'correctAnswer': 'U Rohman (Mehribon) va Rohim (Rahmli)',
    },
    {
      'question': "Maliki yawmid-din oyatining ma'nosi nima?",
      'answers': [
        'U Rohman (Mehribon) va Rohim (Rahmli)',
        'Mehribon va rahmli Alloh nomi bilan (boshlayman)',
        'Hamd olamlar Robbi - Allohgadur',
        'Jazo-mukofot kunining egasidur',
      ],
      'correctAnswer': 'Jazo-mukofot kunining egasidur',
    },
    {
      'question': "Iyyaka na'budu wa iyyaka nasta'in oyatining ma'nosi nima?",
      'answers': [
        "Faqat Sengagina ibodat qilamiz, Sendangina yordam so'raymiz",
        "Bizni to'g'ri yo'lga hidoyat qilgin",
        "O'zing ne'mat berganlarning yo'liga",
        "G'azabga qolganlarnikiga ham emas, adashganlarnikiga ham emas",
      ],
      'correctAnswer':
          "Faqat Sengagina ibodat qilamiz, Sendangina yordam so'raymiz",
    },
    {
      'question': "Ihdinas-sirotol-mustaqim oyatining ma'nosi nima?",
      'answers': [
        "Faqat Sengagina ibodat qilamiz, Sendangina yordam so'raymiz",
        "Bizni to'g'ri yo'lga hidoyat qilgin",
        "O'zing ne'mat berganlarning yo'liga",
        "G'azabga qolganlarnikiga ham emas, adashganlarnikiga ham emas",
      ],
      'correctAnswer': "Bizni to'g'ri yo'lga hidoyat qilgin",
    },
    {
      'question': "Sirotol-laziyna an'amta 'alayhim oyatining ma'nosi nima?",
      'answers': [
        "Faqat Sengagina ibodat qilamiz, Sendangina yordam so'raymiz",
        "Bizni to'g'ri yo'lga hidoyat qilgin",
        "O'zing ne'mat berganlarning yo'liga",
        "G'azabga qolganlarnikiga ham emas, adashganlarnikiga ham emas",
      ],
      'correctAnswer': "O'zing ne'mat berganlarning yo'liga",
    },
    {
      'question':
          "G'oyril-mag'dubi 'alayhim walad-dooliin oyatining ma'nosi nima?",
      'answers': [
        "Faqat Sengagina ibodat qilamiz, Sendangina yordam so'raymiz",
        "Bizni to'g'ri yo'lga hidoyat qilgin",
        "O'zing ne'mat berganlarning yo'liga",
        "G'azabga qolganlarnikiga ham emas, adashganlarnikiga ham emas",
      ],
      'correctAnswer':
          "G'azabga qolganlarnikiga ham emas, adashganlarnikiga ham emas",
    },
    {
      'question': "Subhana Robbiyal-'Azim kalimasining ma'nosi nima?",
      'answers': [
        'Oliy Robbim nuqsonlardan pokdir',
        "Ulug' Robbim nuqsonlardan pokdir",
        'Alloh hamd aytuvchilarni eshitadi',
        "Robbimiz Senga hamd bo'lsin",
      ],
      'correctAnswer': "Ulug' Robbim nuqsonlardan pokdir",
    },
    {
      'question': "Subhana Robbiyal-A'la kalimasining ma'nosi nima?",
      'answers': [
        'Oliy Robbim nuqsonlardan pokdir',
        "Ulug' Robbim nuqsonlardan pokdir",
        'Alloh hamd aytuvchilarni eshitadi',
        "Robbimiz Senga hamd bo'lsin",
      ],
      'correctAnswer': 'Oliy Robbim nuqsonlardan pokdir',
    },
    {
      'question': "Sami' Allohu liman hamidah kalimasining ma'nosi nima?",
      'answers': [
        'Oliy Robbim nuqsonlardan pokdir',
        "Ulug' Robbim nuqsonlardan pokdir",
        'Alloh hamd aytuvchilarni eshitadi',
        "Robbimiz Senga hamd bo'lsin",
      ],
      'correctAnswer': 'Alloh hamd aytuvchilarni eshitadi',
    },
    {
      'question': "Robbana lakal hamd kalimasining ma'nosi nima?",
      'answers': [
        'Oliy Robbim nuqsonlardan pokdir',
        "Ulug' Robbim nuqsonlardan pokdir",
        'Alloh hamd aytuvchilarni eshitadi',
        "Robbimiz Senga hamd bo'lsin",
      ],
      'correctAnswer': "Robbimiz Senga hamd bo'lsin",
    },
    {
      'question': "Wal-'asr oyatining ma'nosi nima?",
      'answers': [
        'Asrga qasamki',
        'Albatta inson ziyon (baxtsizlik)dadir',
        'Illo, imon keltirganlar va solih amal qilganlar',
        "Bir-birlarini haq yo'lga chaqirganlar va bir-birlarini sabrga chaqirganlar (unday emasdir)",
      ],
      'correctAnswer': 'Asrga qasamki',
    },
    {
      'question': "Innal-insana lafi xusr oyatining ma'nosi nima?",
      'answers': [
        'Asrga qasamki',
        'Albatta inson ziyon (baxtsizlik)dadir',
        'Illo, imon keltirganlar va solih amal qilganlar',
        "Bir-birlarini haq yo'lga chaqirganlar va bir-birlarini sabrga chaqirganlar (unday emasdir)",
      ],
      'correctAnswer': 'Albatta inson ziyon (baxtsizlik)dadir',
    },
    {
      'question':
          "Illal-laziyna aamanu wa 'amilus-solihati oyatining ma'nosi nima?",
      'answers': [
        'Asrga qasamki',
        'Albatta inson ziyon (baxtsizlik)dadir',
        'Illo, imon keltirganlar va solih amal qilganlar',
        "Bir-birlarini haq yo'lga chaqirganlar va bir-birlarini sabrga chaqirganlar (unday emasdir)",
      ],
      'correctAnswer': 'Illo, imon keltirganlar va solih amal qilganlar',
    },
    {
      'question':
          "wa tawasow bil-haqqi wa tawasow bis-sobr oyatining ma'nosi nima?",
      'answers': [
        'Asrga qasamki',
        'Albatta inson ziyon (baxtsizlik)dadir',
        'Illo, imon keltirganlar va solih amal qilganlar',
        "Bir-birlarini haq yo'lga chaqirganlar va bir-birlarini sabrga chaqirganlar (unday emasdir)",
      ],
      'correctAnswer':
          "Bir-birlarini haq yo'lga chaqirganlar va bir-birlarini sabrga chaqirganlar (unday emasdir)",
    },
    {
      'question': "Inna a'toynakal-Kavsar oyatining ma'nosi nima?",
      'answers': [
        'Albatta Biz sizga Kavsarni ato etdik',
        "Bas, Robbinga namoz o'qi va jonliq so'y",
        "Albatta, seni yomon ko'rib ayblovchining o'zini orqasi kesik",
        "To'g'ri javob yo'q",
      ],
      'correctAnswer': 'Albatta Biz sizga Kavsarni ato etdik',
    },
    {
      'question': "Fasolli liRobbika wanhar oyatining ma'nosi nima?",
      'answers': [
        'Albatta Biz sizga Kavsarni ato etdik',
        "Bas, Robbinga namoz o'qi va jonliq so'y",
        "Albatta, seni yomon ko'rib ayblovchining o'zini orqasi kesik",
        "To'g'ri javob yo'q",
      ],
      'correctAnswer': "Bas, Robbinga namoz o'qi va jonliq so'y",
    },
    {
      'question': "Inna shaaniaka huwal-abtar oyatining ma'nosi nima?",
      'answers': [
        'Albatta Biz sizga Kavsarni ato etdik',
        "Bas, Robbinga namoz o'qi va jonliq so'y",
        "Albatta, seni yomon ko'rib ayblovchining o'zini orqasi kesik",
        "To'g'ri javob yo'q",
      ],
      'correctAnswer':
          "Albatta, seni yomon ko'rib ayblovchining o'zini orqasi kesik",
    },
    {
      'question': "Qul huw Allohu ahad oyatining ma'nosi nima?",
      'answers': [
        'Ayt: Alloh yagonadir',
        "Alloh - hojatlarni va rag'batlarni qondiruvchidir",
        "U tug'magan va tug'ilmagan",
        'Va Unga hech kim teng emasdir',
      ],
      'correctAnswer': 'Ayt: Alloh yagonadir',
    },
    {
      'question': "Allohu s-somad oyatining ma'nosi nima?",
      'answers': [
        'Ayt: Alloh yagonadir',
        "Alloh - hojatlarni va rag'batlarni qondiruvchidir",
        "U tug'magan va tug'ilmagan",
        'Va Unga hech kim teng emasdir',
      ],
      'correctAnswer': "Alloh - hojatlarni va rag'batlarni qondiruvchidir",
    },
    {
      'question': "Lam yalid wa lam yuulad oyatining ma'nosi nima?",
      'answers': [
        'Ayt: Alloh yagonadir',
        "Alloh - hojatlarni va rag'batlarni qondiruvchidir",
        "U tug'magan va tug'ilmagan",
        'Va Unga hech kim teng emasdir',
      ],
      'correctAnswer': "U tug'magan va tug'ilmagan",
    },
    {
      'question': "Wa lam yakul-lahu kufuwan ahad oyatining ma'nosi nima?",
      'answers': [
        'Ayt: Alloh yagonadir',
        "Alloh - hojatlarni va rag'batlarni qondiruvchidir",
        "U tug'magan va tug'ilmagan",
        'Va Unga hech kim teng emasdir',
      ],
      'correctAnswer': 'Va Unga hech kim teng emasdir',
    },
    {
      'question': "Qul a'uuzu bi Robbil-falaq oyatining ma'nosi nima?",
      'answers': [
        "Ayt: men tong Robbisidan panoh so'rayman",
        "O'zi yaratgan narsalar yomonligidan",
        "Va kirib kelgan qorong'u kechaning yomonligidan",
        'Va tugunlarga dam soluvchilar yomonligidan',
      ],
      'correctAnswer': "Ayt: men tong Robbisidan panoh so'rayman",
    },
    {
      'question': "Min sharri ma xolaq oyatining ma'nosi nima?",
      'answers': [
        "Ayt: men tong Robbisidan panoh so'rayman",
        "O'zi yaratgan narsalar yomonligidan",
        "Va kirib kelgan qorong'u kechaning yomonligidan",
        'Va tugunlarga dam soluvchilar yomonligidan',
      ],
      'correctAnswer': "O'zi yaratgan narsalar yomonligidan",
    },
    {
      'question': "Wa min sharri g'oosiqin iza waqob oyatining ma'nosi nima?",
      'answers': [
        "Ayt: men tong Robbisidan panoh so'rayman",
        "O'zi yaratgan narsalar yomonligidan",
        "Va kirib kelgan qorong'u kechaning yomonligidan",
        'Va tugunlarga dam soluvchilar yomonligidan',
      ],
      'correctAnswer': "Va kirib kelgan qorong'u kechaning yomonligidan",
    },
    {
      'question':
          "Wa min sharrin-naffaa-saati fil 'uqod oyatining ma'nosi nima?",
      'answers': [
        "Ayt: men tong Robbisidan panoh so'rayman",
        "O'zi yaratgan narsalar yomonligidan",
        "Va kirib kelgan qorong'u kechaning yomonligidan",
        'Va tugunlarga dam soluvchilar yomonligidan',
      ],
      'correctAnswer': 'Va tugunlarga dam soluvchilar yomonligidan',
    },
    {
      'question': "Wa min sharri haasidin iza hasad oyatining ma'nosi nima?",
      'answers': [
        'Va hasad qilgan hasadchining yomonligidan',
        "O'zi yaratgan narsalar yomonligidan",
        "Va kirib kelgan qorong'u kechaning yomonligidan",
        'Va tugunlarga dam soluvchilar yomonligidan',
      ],
      'correctAnswer': 'Va hasad qilgan hasadchining yomonligidan',
    },
    {
      'question':
          "Namoz paytida inson ko'ksini yoki boshini qibladan burishining hukmi qanday?",
      'answers': [
        "Ko'ksini burish namozni buzadi, boshni burish makruh",
        'Ikkalasi ham namozni buzadi',
        'Bu harakatlar namozni buzmaydi, ammo makruh',
        'Makruh ham emas, namozni ham buzmaydi',
      ],
      'correctAnswer': "Ko'ksini burish namozni buzadi, boshni burish makruh",
    },
    {
      'question': 'Quydagilarning qaysi biri namozni buzadi?',
      'answers': [
        'Bilib, bilmay yoki adashib gapirish',
        "Uzrsiz yo'talish, tomoq qirqish",
        'Namoz davomida maxs muddatining tugashi',
        "Hamma javob to'g'ri",
      ],
      'correctAnswer': "Hamma javob to'g'ri",
    },
    {
      'question':
          'Quydagilardan qaysi bir namoz vaqtida makruh amal hisoblanadi?',
      'answers': [
        'Erkaklarning sajdada tirsaklarini yerga tegishi',
        "Sajdaga borayotganda kiyimlarni yig'ish, to'g'irlash",
        "Namozda ikkinchi rak'atdagi qiroatni birinchi rak'attan uzun qilish",
        "Hamma javob to'g'ri",
      ],
      'correctAnswer': "Hamma javob to'g'ri",
    },
    {
      'question':
          'Qiyin ahvolda qolgan kishining yordamga chaqirishini eshitganida, yordam berish uchun namozni buzish hukmi nima?',
      'answers': [
        'Harom',
        'Vojib',
        'Sunna',
        'Makruh',
      ],
      'correctAnswer': 'Vojib',
    },
    {
      'question':
          'Farz namoz o‘qiyotgan kishining ota-onasidan biri chaqirganida namozni buzishi joizmi?',
      'answers': [
        'Joiz',
        'Joiz lekin makruh',
        'Namozni buzish Vojib',
        'Joiz emas',
      ],
      'correctAnswer': 'Joiz emas',
    },
    {
      'question': 'Qunut duosini bilmagan inson Vitr namozida nima qiladi?',
      'answers': [
        "Uch marta Allohummag'firli deydi, yoki Robbana atina duosini o'qiydi",
        'Uch marta Alhamdulillah deyishi kifoya',
        'Bir muddat sukut saqlab ruku qiladi',
        "Qunut duosini bilmasa Vitr namozi insondan soqit bo'ladi"
      ],
      'correctAnswer':
          "Uch marta Allohummag'firli deydi, yoki Robbana atina duosini o'qiydi",
    },
    {
      'question': 'Namozda Vojib amalni qasddan tark etishning hukmi nima?',
      'answers': [
        'Namoz buzulmaydi',
        "Namoz buzuladi, qaytadan o'qiladi",
        'Namoz buzulmaydi, faqat sajdai sahv qilinadi',
        "To'g'ri javob yo'q",
      ],
      'correctAnswer': "Namoz buzuladi, qaytadan o'qiladi",
    },
    {
      'question': 'Namozda Vojib amalni bilmasdan tark etishning hukmi nima?',
      'answers': [
        'Namoz buzulmaydi',
        "Namoz buzuladi, qaytadan o'qiladi",
        'Namoz buzulmaydi, faqat sajdai sahv qilinadi',
        "To'g'ri javob yo'q",
      ],
      'correctAnswer': 'Namoz buzulmaydi, faqat sajdai sahv qilinadi',
    },
    {
      'question':
          "Qur'oni Karimdagi sajda oyatlarini o'qigandan keyin Sajdai Tilovat qilish uchun tahorat shartmi?",
      'answers': [
        'Shart emas',
        "Tahorat qilsa yaxshi, qilmasa gunoh yo'q",
        'Tahorat shart',
        "To'g'ri javob yo'q",
      ],
      'correctAnswer': 'Tahorat shart',
    },
    {
      'question': 'Istisqo qanday namoz?',
      'answers': [
        "Quyosh tutilsa o'qiladigan namoz",
        "Oy tutilsa o'qiladigan namoz",
        'Shukr namozi',
        "Yomg'ir so'rab o'qiladigan namoz",
      ],
      'correctAnswer': "Yomg'ir so'rab o'qiladigan namoz",
    },
    {
      'question': "Qul a'uuzu bi Robbin-naas oyatining ma'nosi nima?",
      'answers': [
        "Ayt, men panoh so'rayman barcha insonlarning Robbisidan",
        'Barcha insonlar Podshoxidan',
        'Barcha insonlar Ilohidan',
        "Berkinib, ko'rinib turuvchi vasvasachining yomonligidan",
      ],
      'correctAnswer':
          "Ayt, men panoh so'rayman barcha insonlarning Robbisidan",
    },
    {
      'question': "Malikin-naas oyatining ma'nosi nima?",
      'answers': [
        "Ayt, men panoh so'rayman barcha insonlarning Robbisidan",
        'Barcha insonlar Podshoxidan',
        'Barcha insonlar Ilohidan',
        "Berkinib, ko'rinib turuvchi vasvasachining yomonligidan",
      ],
      'correctAnswer': 'Barcha insonlar Podshoxidan',
    },
    {
      'question': "Ilahin-naas oyatining ma'nosi nima?",
      'answers': [
        "Ayt, men panoh so'rayman barcha insonlarning Robbisidan",
        'Barcha insonlar Podshoxidan',
        'Barcha insonlar Ilohidan',
        "Berkinib, ko'rinib turuvchi vasvasachining yomonligidan",
      ],
      'correctAnswer': 'Barcha insonlar Ilohidan',
    },
    {
      'question': "Min sharril-waswaasil-xonnaas oyatining ma'nosi nima?",
      'answers': [
        "Ayt, men panoh so'rayman barcha insonlarning Robbisidan",
        'Barcha insonlar Podshoxidan',
        'Barcha insonlar Ilohidan',
        "Berkinib, ko'rinib turuvchi vasvasachining yomonligidan",
      ],
      'correctAnswer':
          "Berkinib, ko'rinib turuvchi vasvasachining yomonligidan",
    },
    {
      'question': "Allazi yuwaswisu fi sudurin-naas oyatining ma'nosi nima?",
      'answers': [
        'Odamlar qalblariga vasvasa soladiganning',
        "Jinlar va odamlardan bo'lganning",
        "Odamlarni yo'ldan og'diradiganning",
        "To'g'ri javob yo'q",
      ],
      'correctAnswer': 'Odamlar qalblariga vasvasa soladiganning',
    },
    {
      'question': "Minal-jinnati wan-naas oyatining ma'nosi nima?",
      'answers': [
        'Odamlar qalblariga vasvasa soladiganning',
        "Jinlar va odamlardan bo'lganning",
        "Odamlarni yo'ldan og'diradiganning",
        "To'g'ri javob yo'q",
      ],
      'correctAnswer': "Jinlar va odamlardan bo'lganning",
    },
    {
      'question':
          "Robbana atina fid-dunya hasana, wa fil-aaxiroti hasana, wa qina a'zaaban-naar duosining ma'nosi nima?",
      'answers': [
        'Robbimiz, bizga bu dunyoda ham yaxshilikni',
        'Oxiratda ham yaxshilikni bergin',
        "Va bizni do'zax azobidan asragin",
        'Hamma javobni bu duo ichiga oladi',
      ],
      'correctAnswer': 'Hamma javobni bu duo ichiga oladi',
    },
    {
      'question':
          "Rabbana taqabbal minna innaka antas Samii'ul Aliim duosining ma'nosi nima?",
      'answers': [
        "Ey Robbimiz, bizdan qabul et, albatta Sening O'zing eshituvchi, biluvchi Zotsan",
        'Ey Robbimiz, qalbimizni hidoyatdan adashtirma',
        "Ey Robbimiz, to'g'ri yo'ldan bizlarni adashtirma",
        "To'g'ri javob yo'q",
      ],
      'correctAnswer':
          "Ey Robbimiz, bizdan qabul et, albatta Sening O'zing eshituvchi, biluvchi Zotsan",
    },
    {
      'question': "Alloh taoloning Quddus ismi nima ma'noni bildiradi?",
      'answers': [
        'Barcha ayblardan xoli, noloyiq sifatlardan pok',
        'Hamma narsani qamrab oluvchi',
        "Hamma narsadan o'ta habardor",
        "Hamma narsani ko'ruvchi",
      ],
      'correctAnswer': 'Barcha ayblardan xoli, noloyiq sifatlardan pok',
    },
    {
      'question': "Alloh taoloning Muhaymin ismi nima ma'noni bildiradi?",
      'answers': [
        'Barcha ayblardan xoli, noloyiq sifatlardan pok',
        'Hamma narsani qamrab oluvchi',
        "Hamma narsadan o'ta habardor",
        "Hamma narsani ko'ruvchi",
      ],
      'correctAnswer': 'Hamma narsani qamrab oluvchi',
    },
    {
      'question': "Alloh taoloning Xobiyr ismi nima ma'noni bildiradi?",
      'answers': [
        'Barcha ayblardan xoli, noloyiq sifatlardan pok',
        'Hamma narsani qamrab oluvchi',
        "Hamma narsadan o'ta habardor",
        "Hamma narsani ko'ruvchi",
      ],
      'correctAnswer': "Hamma narsadan o'ta habardor",
    },
    {
      'question': "Alloh taoloning Basiyr ismi nima ma'noni bildiradi?",
      'answers': [
        'Barcha ayblardan xoli, noloyiq sifatlardan pok',
        'Hamma narsani qamrab oluvchi',
        "Hamma narsadan o'ta habardor",
        "Hamma narsani ko'ruvchi",
      ],
      'correctAnswer': "Hamma narsani ko'ruvchi",
    },
  ];
}
